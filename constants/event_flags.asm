...
;Etalon Gym Leaders
	const EVENT_BEAT_TREY
	const EVENT_BEAT_DARIA
	const EVENT_BEAT_ANNABELLE
	const EVENT_BEAT_ERI
	const EVENT_BEAT_TSUBAKI
	const EVENT_BEAT_BEPPI
	const EVENT_BEAT_NERO
	const EVENT_BEAT_LORELEI
	const EVENT_BEAT_KENJI 

;Etalon Gym TMS

	const EVENT_GOT_TM08_BUBBLEBEAM
	const EVENT_GOT_TM35_DAZZLINGLEAM

;Etalon Story Events
	const EVENT_SPOKE_TO_GUARD
	const EVENT_BEAT_HAGA
	const EVENT_CASTOR_WOODS_BEEDRILL_ATTACK_1
	const EVENT_CASTOR_WOODS_BEEDRILL_ATTACK_2
	const EVENT_CASTOR_WOODS_BEEDRILL_ATTACK_3
	const EVENT_CASTOR_WOODS_BEEDRILL_ATTACK_4
	const EVENT_CASTOR_WOODS_BEEDRILL_ATTACK_5
	const EVENT_CASTOR_WOODS_BEEDRILL_ATTACK_6
	const EVENT_CASTOR_WOODS_DANGER
	const EVENT_CLEARED_CASTOR_WOODS
	const EVENT_LORELEI_FOREST_DUNGEON
	const EVENT_FOUGHT_INDIGO
	const EVENT_ROUNDED_UP_STANTLER
	const EVENT_ANNABELLE_LOCKED
	const EVENT_BEAT_COINCASE_GUY
	const ERI_BECOMES_RIVAL
	const EVENT_GOT_SUNSCREEN
	const EVENT_OMEKATA_GYM_GUIDE_OUT_OF_THE_WAY
	
; Etalon Misc Events
	const EVENT_HIDE_HITMONLEE	
	const EVENT_HIDE_HITMONCHAN	
	const EVENT_HIDE_LOPUNNY	
	const EVENT_HIDE_ALAKAZAM	

;Overworld Unowns
	const EVENT_BEAT_DIAMANT_CAVES_UNOWN1 ; Token 1
	const EVENT_BEAT_OMEKATA_DOJO_UNOWN ;token 

;Etalon Items
	const EVENT_CASTOR_WOODS_SUPER_POTION
	const EVENT_CASTOR_WOODS_GREAT_BALL

; Etalon Hidden Items
	const EVENT_CASTOR_WOODS_HIDDEN_SUN_STONE
	const EVENT_CASTOR_WOODS_HIDDEN_SILVER_LEAF_1
	const EVENT_CASTOR_WOODS_HIDDEN_SILVER_LEAF_2
	const EVENT_CASTOR_WOODS_HIDDEN_SILVER_LEAF_3
	const EVENT_CASTOR_WOODS_HIDDEN_SILVER_LEAF_4
	const EVENT_CASTOR_WOODS_HIDDEN_SILVER_LEAF_5
	const EVENT_CASTOR_WOODS_HIDDEN_SILVER_LEAF_6
	const EVENT_CASTOR_WOODS_HIDDEN_GOLD_LEAF
	const EVENT_CASTOR_WOODS_HIDDEN_GOLD_LEAF_2
	const EVENT_CASTOR_WOODS_HIDDEN_GOLD_LEAF_3
	const EVENT_CASTOR_WOODS_HIDDEN_GOLD_LEAF_4
	const EVENT_CASTOR_WOODS_HIDDEN_TINY_MUSHROOM_1
	const EVENT_CASTOR_WOODS_HIDDEN_TINY_MUSHROOM_2
	const EVENT_CASTOR_WOODS_HIDDEN_BIG_MUSHROOM
	const EVENT_CASTOR_WOODS_HIDDEN_MIRACLE_SEED

; Johto story events
	const EVENT_GOT_RALTS
	const EVENT_GRAMPS_BACK_BROKE
	const EVENT_GRAMPS_GIVE_GROWLITHE
	const EVENT_RETURN_GRAMPS_GROWLITHE
	const EVENT_GOT_GROWLITHE
	const EVENT_GIVE_GROWLITHE
	const EVENT_ROUTE48N_SPAWN_GROWLITHE
	const EVENT_COOLTRAINER_GIVE_PINK_BOW
	const EVENT_COOLTRAINER_GIVE_THREE_GREAT_BALL
	const EVENT_MEET_LORELEI_FIRST_TIME
	const EVENT_TALK_WITH_IVY_1
	const EVENT_FACE_RIVAL_FIRST_TIME
	const EVENT_ROUTE_48_ITEM_POKEBALL
	const EVENT_ROUTE_48_ITEM_POTION
	const EVENT_FOUGHT_AVULPIX
	const EVENT_ROUTE_48_ITEM_ANTIDOTE
	const EVENT_GOT_TM_PLACEHOLDER
	const EVENT_ROUTE_48S_POTION
	const EVENT_ROUTE_48S_POKE_BALL
	const EVENT_ROUTE_48S_ANTIDOTE
	const EVENT_ROUTE_48S_CALCIUM
	const EVENT_GOT_A_POKEMON_FROM_ELM
	const EVENT_GOT_CYNDAQUIL_FROM_ELM
	const EVENT_GOT_TOTODILE_FROM_ELM
	const EVENT_GOT_ABRA_FROM_ELM
	...
	const EVENT_ERABLE_TOWN_HIDDEN_FULL_HEAL
	const EVENT_ERABLE_TOWN_HIDDEN_BERRY
	const EVENT_ROUTE_49_HIDDEN_TINY_MUSHROOM_ONE
	const EVENT_ROUTE_49_HIDDEN_TINY_MUSHROOM_TWO
	const EVENT_BRIDGEN_HIDDEN_ULTRA_BALL
	const EVENT_BRIDGEN_HIDDEN_PROTEIN
	const EVENT_MOUETTE_HIDDEN_GOLD_BERRY
	const EVENT_MOUETTE_HIDDEN_NUGGET
	const EVENT_MOUETTE_HIDDEN_RARE_CANDY
	const EVENT_MOUETTE_HIDDEN_GOLD_BERRY2
	const EVENT_MOUETTE_HIDDEN_MINT_BERRY
	const EVENT_MOUETTE_HIDDEN_MINT_BERRY2
	const EVENT_OMEKATA_HIDDEN_CALCIUM
	const EVENT_OMEKATA_HIDDEN_GOLD_BERRY
	const EVENT_OMEKATA_HIDDEN_MIRACLE_BERRY
	const EVENT_OMEKATA_HIDDEN_RARE_CANDY
	const EVENT_OMEKATA_HIDDEN_CARBOS
	const EVENT_OMEKATA_HIDDEN_MUSHROOM
	const EVENT_OMEKATA_HIDDEN_FIRE_STONE
	const EVENT_OMEKATA_HIDDEN_MAX_REVIVE
...
; Hidden Etalon Items
	const EVENT_ROUTE_48N_HIDDEN_BERRY
	const EVENT_ROUTE_48N_HIDDEN_GREATBALL
	const EVENT_ROUTE_48N_HIDDEN_GREATBALL2
	const EVENT_ROUTE_48N_HIDDEN_REPEL
	const EVENT_ROUTE_48N_HIDDEN_SILVERLEAF
	const EVENT_ROUTE_48S_HIDDEN_XSPECIAL
	const EVENT_ROUTE_48S_HIDDEN_POKEBALL
	const EVENT_ROUTE_48S_HIDDEN_BERRY
...
; Etalon Misc Events
	const EVENT_GAVE_TIPS
	const EVENT_GOT_COINS_FROM_ARCHER
	const EVENT_GOT_COINS_FROM_ROCKET
	const EVENT_GOT_COINS_FROM_ROCKET2
	const EVENT_GOT_COINS_FROM_GAMBLER
	const EVENT_GOT_COINS_FROM_GAMBLER2
	const EVENT_GOT_COINS_FROM_GAMBLER3
	const EVENT_GOT_COINS_FROM_GAMBLER4
	const EVENT_GOT_COINS_FROM_BEAUTY
	const EVENT_GOT_COINS_FROM_BEAUTY2
	const EVENT_GOT_COINS_FROM_BEAUTY3
	const EVENT_GOT_COINS_FROM_GENTLEMAN
	const EVENT_GOT_COINS_FROM_GENTLEMAN2
	const EVENT_GOT_COINS_FROM_COOLTRAINERF
	const EVENT_GOT_COINS_FROM_COOLTRAINERM
	const EVENT_GOSHA_CITY_BEAUTY_COINS
	const EVENT_GOSHA_GOT_MUGGED

; Gift Pokemon
	const EVENT_SAVED_AVULPIX
	const EVENT_BOUGHT_DRATINI
	const EVENT_BOUGHT_CHARMANDER
	const EVENT_BOUGHT_TOTODILE
	const EVENT_BOUGHT_TURTWIG

; Shiny OW Pokemon
	const EVENT_FOUGHT_YANMA
	const EVENT_FOUGHT_MILOTIC
	const EVENT_FOUGHT_BD_WEEDLE
	const EVENT_FOUGHT_POLIWRATH
	const EVENT_FOUGHT_LUDICOLO
...
; Biker
	...
	const EVENT_BEAT_BIKER_BUZZ
	const EVENT_BEAT_BIKER_CROW
	const EVENT_BEAT_BIKER_BUTCH
	const EVENT_BEAT_BIKER_TINY
...
; Gentleman
	...
	const EVENT_BEAT_GENTLEMAN_MIGUEL
...
; Blackbelt
	...
	const EVENT_BEAT_BLACKBELT_HONDA
	const EVENT_BEAT_BLACKBELT_RIKI
;BattleGirl
	const EVENT_BEAT_BATTLEGIRL_SAM
	const EVENT_BEAT_BATTLEGIRL_KYOKO
; Beauty
	const EVENT_BEAT_BEAUTY_LISA
	const EVENT_BEAT_BEAUTY_LIA 
	const EVENT_BEAT_BEAUTY_JESSICA
	const EVENT_BEAT_BEUATY_ALEXIS
;Gambler
	const EVENT_BEAT_GAMBLER_PIERRE
	const EVENT_BEAT_GAMBLER_KARL
	const EVENT_BEAT_GAMBLER_PHILIPPE
	const EVENT_BEAT_GAMBLER_JORDAN
; Johto Gym Leaders
...
; PokefanF
	const EVENT_BEAT_POKEFANF_ROSALIE
; Kimono Girl
	const_skip ; unused
...
;JamzM
	const EVENT_BEAT_JAMZM1
	const EVENT_BEAT_JAMZM2
	const EVENT_BEAT_JAMZM3
	const EVENT_BEAT_JAMZM4
;JamzF
	const EVENT_BEAT_JAMZF1
	const EVENT_BEAT_JAMZF2
; GruntM
...
; Lass
	const EVENT_BEAT_LASS_NIEVA
	const EVENT_BEAT_LASS_VALERIE
	const EVENT_BEAT_LASS_SHANNON2
	const EVEN_BEAT_LASS_MARIE
	const EVENT_BEAT_LASS_RIKKA
; Hiker
...
; Bug Catcher
	...
	const EVENT_BEAT_BUG_CATCHER_HUGO
	const EVENT_BEAT_BUG_CATCHER_WES
	const EVENT_BEAT_BUG_CATCHER_GAEL
...
; CooltrainerM
	...
	const EVENT_BEAT_COOLTRAINERM_DARWIN
	const EVENT_BEAT_GYM_TRAINER_FERN
	const EVENT_BEAT_GYM_TRAINER_KAI
; CooltrainerF
	...
	const EVENT_BEAT_GYM_TRAINER_AVRIL
; ExecutiveF
...
; Sailor
	const EVENT_BEAT_SAILOR_DAVEY
	const EVENT_BEAT_SAILOR_HURLEY
...
; Super Nerd
	...
	const EVENT_BEAT_SUPER_NERD_WILLIAM
...
; Youngster
	...
	const EVENT_BEAT_YOUNGSTER_LARRY
	const EVENT_BEAT_YOUNGSTER_JONAS
	const EVENT_BEAT_YOUNGSTER_THEO2
; Teacher
	const EVENT_BEAT_TEACHER_COLETTE
	const EVENT_BEAT_TEACHER_HILLARY
	const EVENT_BEAT_TEACHER_SHIRLEY
;Nurse
	const EVENT_BEAT_NURSE_ELLIOT
;Cue Ball 
	const EVENT_BEAT_CUEBALL_BASS
	const EVENT_BEAT_CUEBALL_RIFF
	const EVENT_BEAT_CUEBALL_MAC
	const EVENT_BEAT_CUEBALL_ALYX
	const EVENT_BEAT_CUEBALL_ARCHIE
...
; Johto itemballs
	...
	const EVENT_ABRA_POKEBALL_IN_ELMS_LAB
	...
	const EVENT_OLIVINE_LIGHTHOUSE_5F_TM_HEADBUTT
	...
	const EVENT_ROUTE_47_POTION
	const EVENT_FOREST_DUNGEON_2F_ITEM1
	const EVENT_FOREST_DUNGEON_2F_ITEM2
	const EVENT_FOREST_DUNGEON_3F_ITEM1
...
; New to Crystal
	...
	const EVENT_ROUTE_49_REPEL
	const EVENT_ROUTE_49_PRZBERRY
	const EVENT_ROUTE_49_ESCAPE_ROPE
	const EVENT_ROUTE_49_BERRY
