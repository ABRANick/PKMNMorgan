; Characteristics of each move.

move: MACRO
	db \1 ; animation
	db \2 ; effect
	db \3 ; power
	db \4 | \5 ; type
	db \6 percent ; accuracy
	db \7 ; pp
	db \8 percent ; effect chance
ENDM

Moves:
; entries correspond to constants/move_constants.asm
	move POUND,        EFFECT_NORMAL_HIT,         40, NORMAL,       PHYSICAL, 100, 30,   0
	move ATTRACT,      EFFECT_ATTRACT,             0, NORMAL,       STATUS, 100, 15,   0
	move BATON_PASS,   EFFECT_BATON_PASS,          0, NORMAL,       STATUS, 100, 40,   0
	move BELLY_DRUM,   EFFECT_BELLY_DRUM,          0, NORMAL,       STATUS, 100, 10,   0
	move BIDE,         EFFECT_BIDE,         	   0, NORMAL,       PHYSICAL, 100, 10,   0
	move BODY_SLAM,    EFFECT_PARALYZE_HIT,       85, NORMAL,       PHYSICAL, 100, 15,  30
	move CAPTIVATE,    EFFECT_SP_ATK_DOWN,         0, NORMAL,        STATUS, 100, 20,   0
	move CONVERSION,   EFFECT_CONVERSION,          0, NORMAL,      	PHYSICAL, 100, 30,   0
	move CONVERSION2,  EFFECT_CONVERSION2,         0, NORMAL,      	PHYSICAL, 100, 30,   0
	move COVET,        EFFECT_THIEF,              75, NORMAL,       PHYSICAL, 100, 10, 100
	move CRUSH_CLAW,   EFFECT_DEFENSE_DOWN_HIT,   75, NORMAL,       PHYSICAL, 95, 15,  50
	move DEFENSE_CURL, EFFECT_DEFENSE_CURL,        0, NORMAL,      	STATUS, 100, 40,   0
	move DOUBLE_EDGE,  EFFECT_RECOIL_HIT,        130, NORMAL,       PHYSICAL, 100, 15,   0
	move ECHOED_VOICE, EFFECT_NORMAL_HIT,         40, NORMAL,       SPECIAL, 100, 30,   0
	move ENCORE,       EFFECT_ENCORE,              0, NORMAL,       STATUS, 100,  5,   0
	move ENDURE,       EFFECT_ENDURE,         	   0, NORMAL,       STATUS, 100, 10,   0
	move EXPLOSION,    EFFECT_SELFDESTRUCT,      250, NORMAL,      	PHYSICAL, 100,  5,   0
	move EXTREMESPEED, EFFECT_PRIORITY_HIT,       80, NORMAL,       PHYSICAL, 100,  5,   0
	move FALSE_SWIPE,  EFFECT_FALSE_SWIPE,        40, NORMAL,       PHYSICAL, 100, 30,   0
	move FLAIL,        EFFECT_REVERSAL,            1, NORMAL,       PHYSICAL, 100, 15,   0
	move FLASH,        EFFECT_ACCURACY_DOWN_HIT,  35, NORMAL,    	SPECIAL,  90, 20,  100
	move FOCUS_ENERGY, EFFECT_FOCUS_ENERGY,        0, NORMAL,       STATUS, 100, 30,   0
	move FURY_ATTACK,  EFFECT_MULTI_HIT,          35, NORMAL,       PHYSICAL, 85, 20,   0
	move GLARE,        EFFECT_PARALYZE,            0, NORMAL,       STATUS, 75, 30,   0
	move GROWL,        EFFECT_ATTACK_DOWN,         0, NORMAL,       STATUS, 100, 40,   0
	move HEADBUTT,     EFFECT_FLINCH_HIT,         70, NORMAL,       PHYSICAL, 100, 15,  30
	move HEAL_BELL,    EFFECT_HEAL_BELL,           0, NORMAL,       STATUS, 100,  5,   0
	move HIDDEN_POWER, EFFECT_HIDDEN_POWER,        1, NORMAL,       SPECIAL, 100, 15,   0
	move HORN_ATTACK,  EFFECT_NORMAL_HIT,         65, NORMAL,       PHYSICAL, 100, 25,   0
	move HOWL,	       EFFECT_ATTACK_UP,           0, NORMAL,		STATUS, 100, 20,   0
	move HYPER_BEAM,   EFFECT_HYPER_BEAM,        160, NORMAL,       PHYSICAL, 90,  5,   0
	move HYPER_VOICE,  EFFECT_NORMAL_HIT,         90, NORMAL,       SPECIAL, 100, 15,   0
	move LEER,         EFFECT_DEFENSE_DOWN,        0, NORMAL,       STATUS, 100, 30,   0
	move LOCK_ON,      EFFECT_LOCK_ON,             0, NORMAL,       STATUS, 100,  5,   0
	move METRONOME,    EFFECT_METRONOME,           0, NORMAL,       STATUS, 100, 10,   0
	move MIMIC,        EFFECT_MIMIC,               0, NORMAL,       STATUS, 100, 10,   0
	move PAY_DAY,      EFFECT_PAY_DAY,            95, NORMAL,       SPECIAL, 100, 20,   0
	move PAIN_SPLIT,   EFFECT_PAIN_SPLIT,          0, NORMAL,       STATUS, 100, 20,   0
	move PERISH_SONG,  EFFECT_PERISH_SONG,         0, NORMAL,       STATUS, 100,  5,   0
	move PROTECT,      EFFECT_PROTECT,             0, NORMAL,       STATUS, 100, 10,   0
	move QUICK_ATTACK, EFFECT_PRIORITY_HIT,       40, NORMAL,       PHYSICAL, 100, 30,   0
	move RAGE,         EFFECT_RAGE,               20, NORMAL,      	PHYSICAL, 100, 20,   0
	move RAPID_SPIN,   EFFECT_RAPID_SPIN,         50, NORMAL,       PHYSICAL, 100, 40,   0
	move RECOVER,      EFFECT_HEAL,                0, NORMAL,       STATUS, 100, 20,   0
	move RETURN,       EFFECT_RETURN,              1, NORMAL,       PHYSICAL, 100, 20,   0
	move REST,         EFFECT_HEAL,                0, NORMAL,	    STATUS, 100, 10,   0
	move ROAR,         EFFECT_FORCE_SWITCH,        0, NORMAL,      	STATUS, 100, 20,   0
	move SAFEGUARD,    EFFECT_SAFEGUARD,           0, NORMAL,       STATUS, 100, 25,   0
	move SCARY_FACE,   EFFECT_SPEED_DOWN_2,        0, NORMAL,       STATUS, 95, 40,   0
	move SCREECH,      EFFECT_DEFENSE_DOWN_2,      0, NORMAL,       STATUS, 85, 40,   0
	move SELFDESTRUCT, EFFECT_SELFDESTRUCT,      200, NORMAL,      	PHYSICAL, 100,  5,   0
	move SING,         EFFECT_SLEEP,               0, NORMAL,       STATUS, 55, 15,   0
	move SLASH,        EFFECT_NORMAL_HIT,         70, NORMAL,       PHYSICAL, 100, 20,   0
	move SLEEP_TALK,   EFFECT_SLEEP_TALK,          0, NORMAL,       STATUS, 100, 10,   0  
	move SOFTBOILED,   EFFECT_HEAL,                0, NORMAL,       STATUS, 100, 10,   0
	move SUBSTITUTE,   EFFECT_SUBSTITUTE,          0, NORMAL,       STATUS, 100, 10,   0
	move SUPER_FANG,   EFFECT_SUPER_FANG,          1, NORMAL,       PHYSICAL, 90, 10,   0
	move STOMP,        EFFECT_STOMP,              65, NORMAL,       PHYSICAL, 100, 20,  50
	move SWAGGER,      EFFECT_SWAGGER,             0, NORMAL,       STATUS, 90, 15, 100
	move SWIFT,        EFFECT_ALWAYS_HIT,         60, NORMAL,       SPECIAL, 100, 20,   0
	move SWORDS_DANCE, EFFECT_ATTACK_UP_2,         0, NORMAL,      	STATUS, 100, 30,   0
	move TACKLE,       EFFECT_NORMAL_HIT,         40, NORMAL,       PHYSICAL, 100, 35,   0
	move TAKE_DOWN,    EFFECT_RECOIL_HIT,         90, NORMAL,       PHYSICAL, 85, 20,   0
	move THRASH,       EFFECT_RAMPAGE,           120, NORMAL,       PHYSICAL, 100, 20,   0
	move TRI_ATTACK,   EFFECT_TRI_ATTACK,         80, NORMAL,       PHYSICAL, 100, 10,  20
	move WHIRLWIND,    EFFECT_FORCE_SWITCH,        0, NORMAL,      	STATUS, 100, 20,   0
	move WRAP,         EFFECT_TRAP_TARGET,        35, NORMAL,       PHYSICAL, 85, 20,   0
	move BLAZE_KICK,   EFFECT_BURN_HIT,           85, FIRE,         PHYSICAL, 90, 15,  10
	move EMBER,        EFFECT_BURN_HIT,           40, FIRE,         SPECIAL, 100, 25,  10
	move FIRE_BLAST,   EFFECT_BURN_HIT,          120, FIRE,         SPECIAL,  85,  5,  10
	move FIRE_PUNCH,   EFFECT_BURN_HIT,           75, FIRE,         PHYSICAL, 100, 15,  10
	move FIRE_SPIN,    EFFECT_TRAP_TARGET,        35, FIRE,         SPECIAL, 85, 15,   0
	move FLAME_WHEEL,  EFFECT_FLAME_WHEEL,        65, FIRE,         PHYSICAL, 100, 25,  10
	move FLAMETHROWER, EFFECT_BURN_HIT,           95, FIRE,         SPECIAL, 100, 15,  10
	move FLARE_BLITZ,  EFFECT_RECOIL_HIT,        120, FIRE,         PHYSICAL, 100, 15,   0
	move HEAT_WAVE,    EFFECT_BURN_HIT,        	 95, FIRE,         SPECIAL,  90, 10,   30
	move MORNING_SUN,  EFFECT_SYNTHESIS,           0, FIRE,         STATUS, 100,  5,   0
	move MYSTICALFIRE, EFFECT_SP_ATK_DOWN_HIT,	  75, FIRE,         SPECIAL, 100, 15,   100
	move SUNNY_DAY,    EFFECT_SUNNY_DAY,           0, FIRE,         STATUS, 100,  5,   0
	move WILL_O_WISP,  EFFECT_BURN_HIT,	       	  20, FIRE,		    SPECIAL, 75, 10,   100
	move AQUA_JET,     EFFECT_PRIORITY_HIT,       40, WATER,        PHYSICAL, 100, 30,   0
	move BUBBLEBEAM,   EFFECT_SPEED_DOWN_HIT,     65, WATER,       	SPECIAL, 100, 20,  10
	move HYDRO_PUMP,   EFFECT_THUNDER,           120, WATER,        SPECIAL, 80,  5,   0
	move RAIN_DANCE,   EFFECT_RAIN_DANCE,          0, WATER,        STATUS, 100,  5,   0
	move SURF,         EFFECT_NORMAL_HIT,         95, WATER,        SPECIAL, 100, 15,   0
	move SCALD,        EFFECT_BURN_HIT,           80, WATER,         SPECIAL,  100,  15,  30
	move WATER_GUN,    EFFECT_NORMAL_HIT,         40, WATER,        SPECIAL, 100, 25,   0
	move WATERFALL,    EFFECT_FLINCH_HIT,         80, WATER,        PHYSICAL, 100, 15,   15
	move WHIRLPOOL,    EFFECT_TRAP_TARGET,        35, WATER,        SPECIAL, 85, 15,   0
	move ABSORB,       EFFECT_LEECH_HIT,          20, GRASS,        SPECIAL, 100, 20,   0
	move ENERGY_BALL,  EFFECT_SP_DEF_DOWN_HIT,    90, GRASS,        SPECIAL, 100, 10,  10
	move GIGA_DRAIN,   EFFECT_LEECH_HIT,          75, GRASS,        SPECIAL, 100, 10,   0
	move GROWTH,       EFFECT_SP_ATK_UP,           0, GRASS,      	STATUS, 100, 40,   0
	move LEAF_BLADE,   EFFECT_NORMAL_HIT,         90, GRASS,        PHYSICAL, 100, 10,   0
	move LEECH_SEED,   EFFECT_LEECH_SEED,          0, GRASS,        STATUS, 90, 10,   0
	move MAGICAL_LEAF, EFFECT_ALWAYS_HIT,         60, GRASS,        SPECIAL, 100, 20,   0
	move MEGA_DRAIN,   EFFECT_LEECH_HIT,          40, GRASS,        SPECIAL, 100, 15,   0
	move PETAL_DANCE,  EFFECT_RAMPAGE,           120, GRASS,       	SPECIAL, 100, 20,   0
	move RAZOR_LEAF,   EFFECT_NORMAL_HIT,         55, GRASS,        PHYSICAL, 95, 25,   0
	move SEED_BOMB,    EFFECT_NORMAL_HIT,         80, GRASS,        PHYSICAL, 100, 15,   0
	move SEED_FLARE,   EFFECT_SP_DEF_DOWN_HIT,   120, GRASS,        SPECIAL, 85, 5,  40
	move SOLARBEAM,    EFFECT_SOLARBEAM,         120, GRASS,        SPECIAL, 100, 10,   0
	move SLEEP_POWDER, EFFECT_SLEEP,               0, GRASS,        STATUS, 75, 15,   0
	move STUN_SPORE,   EFFECT_PARALYZE,            0, GRASS,        STATUS, 75, 30,   0
	move VINE_WHIP,    EFFECT_NORMAL_HIT,         40, GRASS,        PHYSICAL, 100, 15,   0
	move SHOCK_WAVE,   EFFECT_ALWAYS_HIT,         60, ELECTRIC,     SPECIAL, 100, 20,   0
	move SPARK,        EFFECT_PARALYZE_HIT,       65, ELECTRIC,     PHYSICAL, 100, 20,  30
	move THUNDER,      EFFECT_THUNDER,           120, ELECTRIC,     SPECIAL, 70, 10,  30
	move THUNDER_WAVE, EFFECT_PARALYZE,            0, ELECTRIC,     STATUS, 100, 20,   0
	move THUNDERBOLT,  EFFECT_PARALYZE_HIT,       95, ELECTRIC,     SPECIAL, 100, 15,  10
	move THUNDERPUNCH, EFFECT_PARALYZE_HIT,       75, ELECTRIC,     PHYSICAL, 100, 15,  10
	move THUNDERSHOCK, EFFECT_PARALYZE_HIT,       40, ELECTRIC,     SPECIAL, 100, 30,  10
	move VOLT_TACKLE,  EFFECT_RECOIL_HIT,        120, ELECTRIC,     PHYSICAL, 100, 15,   0
	move WILD_CHARGE,  EFFECT_RECOIL_HIT,         90, ELECTRIC,     PHYSICAL, 100, 20,   0
	move ZAP_CANNON,   EFFECT_PARALYZE_HIT,      150, ELECTRIC,     SPECIAL, 50,  5, 100
	move AURORA_BEAM,  EFFECT_ATTACK_DOWN_HIT,    65, ICE,          SPECIAL, 100, 20,  10
	move BLIZZARD,     EFFECT_BLIZZARD,        	 120, ICE,          SPECIAL, 85,  5,  10
	move HAIL,         EFFECT_HAIL,           	   0, ICE,          STATUS, 100, 10,   0
	move HAZE,         EFFECT_RESET_STATS,         0, ICE,          STATUS, 100, 30,   0
	move ICE_BEAM,     EFFECT_FREEZE_HIT,         95, ICE,          SPECIAL, 100, 10,  10
	move ICE_PUNCH,    EFFECT_FREEZE_HIT,         75, ICE,          PHYSICAL, 100, 15,  10
	move ICE_SHARD,    EFFECT_PRIORITY_HIT,       40, ICE,          PHYSICAL, 100, 30,   0
	move ICICLECRASH,  EFFECT_FLINCH_HIT,         85, ICE,          PHYSICAL, 90, 10,  30
	move POWDER_SNOW,  EFFECT_FREEZE_HIT,         40, ICE,          SPECIAL, 100, 25,  10
	move AURA_SPHERE,  EFFECT_ALWAYS_HIT,         80, FIGHTING,     SPECIAL, 100, 20,   0
	move BULK_UP,      EFFECT_BULK_UP,             0, FIGHTING,     STATUS, 100, 20,   0
	move COUNTER,      EFFECT_COUNTER,             1, FIGHTING,     PHYSICAL, 100, 20,   0
	move CLOSECOMBAT,  EFFECT_CLOSE_COMBAT,       120, FIGHTING,    PHYSICAL, 100,  5,   0
	move CROSS_CHOP,   EFFECT_NORMAL_HIT,        100, FIGHTING,     PHYSICAL, 80,  5,   0
	move DOUBLE_KICK,  EFFECT_DOUBLE_HIT,         30, FIGHTING,     PHYSICAL, 100, 30,   0
	move DRAIN_PUNCH,  EFFECT_LEECH_HIT,          60, FIGHTING,     PHYSICAL, 100, 10,   0
	move DYNAMICPUNCH, EFFECT_CONFUSE_HIT,       100, FIGHTING,     PHYSICAL, 50,  5, 100
	move FORCE_PALM,   EFFECT_PARALYZE_HIT,       60, FIGHTING,     PHYSICAL, 100, 15,  30
	move REVERSAL,     EFFECT_REVERSAL,            1, FIGHTING,     PHYSICAL, 100, 15,   0
	move HI_JUMP_KICK, EFFECT_JUMP_KICK,         110, FIGHTING,    PHYSICAL, 90, 20,   0
	move JUMP_KICK,    EFFECT_JUMP_KICK,          75, FIGHTING,     PHYSICAL, 95, 25,   0
	move KARATE_CHOP,  EFFECT_NORMAL_HIT,         50, FIGHTING,     PHYSICAL, 100, 25,   0
	move LOW_SWEEP,    EFFECT_SPEED_DOWN_HIT,     65, FIGHTING,     PHYSICAL, 100, 20,  100
	move MACH_PUNCH,   EFFECT_PRIORITY_HIT,       40, FIGHTING,     PHYSICAL, 100, 30,   0
	move ROCK_SMASH,   EFFECT_DEFENSE_DOWN_HIT,   75, FIGHTING,     PHYSICAL, 100, 15,  10
	move SEISMIC_TOSS, EFFECT_LEVEL_DAMAGE,        1, FIGHTING,     PHYSICAL, 100, 20,   0
	move STRENGTH,     EFFECT_NORMAL_HIT,         80, FIGHTING,     PHYSICAL, 100, 15,   0
	move ACID,         EFFECT_SP_DEF_DOWN_HIT,    40, POISON,       SPECIAL, 100, 30,  10
	move GUNK_SHOT,    EFFECT_POISON_HIT,        120, POISON,       PHYSICAL, 85, 10,  30
	move POISON_JAB,   EFFECT_POISON_HIT,         90, POISON,       PHYSICAL, 100, 10,  30
	move POISON_STING, EFFECT_POISON_HIT,         35, POISON,       PHYSICAL, 100, 35,  30
	move POISON_TAIL,  EFFECT_POISON_HIT,         65, POISON,       PHYSICAL, 100, 35,  30
	move SMOG,         EFFECT_POISON_HIT,         35, POISON,       SPECIAL, 70, 20,  40
	move SLUDGE,       EFFECT_POISON_HIT,         65, POISON,       SPECIAL, 100, 20,  30
	move SLUDGE_BOMB,  EFFECT_POISON_HIT,         90, POISON,       SPECIAL, 100, 10,  30
	move TOXIC,        EFFECT_TOXIC,               0, POISON,       STATUS, 85, 10,   0
	move BONE_CLUB,    EFFECT_FLINCH_HIT,         65, GROUND,       PHYSICAL, 85, 20,  10
	move BONEMERANG,   EFFECT_DOUBLE_HIT,         50, GROUND,       PHYSICAL, 90, 10,   0
	move BULLDOZE,     EFFECT_SPEED_DOWN_HIT,     60, GROUND,       SPECIAL, 95, 15, 100
	move DIG,          EFFECT_FLY,                80, GROUND,       PHYSICAL, 100, 10,   0
	move EARTH_POWER,  EFFECT_NORMAL_HIT,         80, GROUND,       SPECIAL, 100, 20,   0
	move EARTHQUAKE,   EFFECT_EARTHQUAKE,        100, GROUND,       PHYSICAL, 100, 10,   0
	move MAGNITUDE,    EFFECT_MAGNITUDE,           1, GROUND,       PHYSICAL, 100, 30,   0
	move MUD_SHOT,     EFFECT_SPEED_DOWN_HIT,     55, GROUND,     	SPECIAL, 85, 10,  50
	move MUD_SLAP,     EFFECT_ACCURACY_DOWN_HIT,  20, GROUND,       SPECIAL, 100, 10, 100
	move SPIKES,       EFFECT_SPIKES,              0, GROUND,       STATUS, 100, 20,   0
	move AERIAL_ACE,   EFFECT_ALWAYS_HIT,         60, FLYING,       PHYSICAL, 100, 20,  0
	move AIR_CUTTER,   EFFECT_NORMAL_HIT,         60, FLYING,       SPECIAL, 95, 20,   0
	move AIR_SLASH,    EFFECT_FLINCH_HIT,         75, FLYING,       SPECIAL, 95, 20,   0
	move BRAVE_BIRD,   EFFECT_RECOIL_HIT,        120, FLYING,       PHYSICAL, 100, 15,   0
	move DRILL_PECK,   EFFECT_NORMAL_HIT,         80, FLYING,       PHYSICAL, 100, 20,   0
	move FLY,          EFFECT_FLY,                90, FLYING,       PHYSICAL, 95, 15,   0
	move GUST,         EFFECT_GUST,               40, FLYING,       SPECIAL, 100, 35,   0
	move HURRICANE,    EFFECT_THUNDER,           110, FLYING,       SPECIAL, 80, 10,  30
	move PECK,         EFFECT_NORMAL_HIT,        40, FLYING,        PHYSICAL, 100, 35,   0
	move AGILITY,      EFFECT_SPEED_UP_2,         0, PSYCHIC_TYPE,  STATUS, 100, 30,   0
	move AMNESIA,      EFFECT_SP_DEF_UP_2,        0, PSYCHIC_TYPE,  STATUS, 100, 20,   0
	move BARRIER,      EFFECT_DEFENSE_UP_2,       0, PSYCHIC_TYPE,  STATUS, 100, 30,   0
	move CALM_MIND,    EFFECT_CALM_MIND,          0, PSYCHIC_TYPE,  STATUS, 100, 20,   0
	move CONFUSION,    EFFECT_CONFUSE_HIT,        50, PSYCHIC_TYPE, SPECIAL, 100, 25,  10
	move DREAM_EATER,  EFFECT_DREAM_EATER,      120, PSYCHIC_TYPE, SPECIAL, 100, 15,   0
	move FUTURE_SIGHT, EFFECT_FUTURE_SIGHT,      200, PSYCHIC_TYPE,	SPECIAL,   90, 15,   0
	move HYPNOSIS,     EFFECT_SLEEP,              0, PSYCHIC_TYPE, STATUS, 60, 20,   0
	move LIGHT_SCREEN, EFFECT_LIGHT_SCREEN,       0, PSYCHIC_TYPE, STATUS, 100, 30,   0
	move MIRROR_COAT,  EFFECT_MIRROR_COAT,        1, PSYCHIC_TYPE, SPECIAL, 100, 20,   0
	move PSYBEAM,      EFFECT_CONFUSE_HIT,        65, PSYCHIC_TYPE, SPECIAL, 100, 20,  10
	move PSYCHIC_M,    EFFECT_SP_DEF_DOWN_HIT,    90, PSYCHIC_TYPE, SPECIAL, 100, 10,  10
	move REFLECT,      EFFECT_REFLECT,             0, PSYCHIC_TYPE, STATUS, 100, 20,   0
	move TELEPORT,     EFFECT_BATON_PASS,          0, PSYCHIC_TYPE, STATUS, 100, 20,   0
	move ZENHEADBUTT,  EFFECT_FLINCH_HIT,         85, PSYCHIC_TYPE, PHYSICAL, 95, 15,  15
	move BUG_BUZZ,     EFFECT_SP_DEF_DOWN_HIT,    90, BUG,          SPECIAL, 100, 10,  10
	move FURY_CUTTER,  EFFECT_FURY_CUTTER,        40, BUG,          PHYSICAL, 95, 20,   0
	move LEECH_LIFE,   EFFECT_LEECH_HIT,          75, BUG,          PHYSICAL, 100, 15,   0
	move MEGAHORN,     EFFECT_NORMAL_HIT,        120, BUG,          PHYSICAL, 85, 10,   0
	move PIN_MISSILE,  EFFECT_MULTI_HIT,          35, BUG,          PHYSICAL, 85, 20,   0
	move SIGNAL_BEAM,  EFFECT_CONFUSE_HIT,        75, BUG,          SPECIAL, 100, 15, 10
	move STRING_SHOT,  EFFECT_SPEED_DOWN,          0, BUG,          STATUS, 95, 40,   0
	move X_SCISSOR,    EFFECT_NORMAL_HIT,         80, BUG,          PHYSICAL, 100, 20,   0
	move TWINEEDLE,    EFFECT_POISON_MULTI_HIT,   50, BUG,          PHYSICAL, 100, 20,  20
	move ANCIENTPOWER, EFFECT_ALL_UP_HIT,         60, ROCK,         SPECIAL, 100,  5,  10
	move HEAD_SMASH,   EFFECT_RECOIL_HIT,        150, ROCK,         PHYSICAL, 80, 5,   0
	move POWER_GEM,    EFFECT_NORMAL_HIT,         80, ROCK,         SPECIAL, 100, 20,   0
	move ROCK_SLIDE,   EFFECT_FLINCH_HIT,         80, ROCK,         PHYSICAL, 90, 10,  30
	move ROCK_THROW,   EFFECT_NORMAL_HIT,         50, ROCK,         PHYSICAL, 85, 15,   0
	move ROCK_TOMB,    EFFECT_SPEED_DOWN_HIT,     60, ROCK,       	PHYSICAL, 90, 10,  100
	move ROLLOUT,      EFFECT_ROLLOUT,            30, ROCK,         PHYSICAL, 90, 20,   0
	move SANDSTORM,    EFFECT_SANDSTORM,           0, ROCK,         STATUS, 100, 10,   0
	move STONE_EDGE,   EFFECT_NORMAL_HIT,        100, ROCK,         PHYSICAL, 80,  5,   0
	move CONFUSE_RAY,  EFFECT_CONFUSE,             0, GHOST,        STATUS, 100, 10,   0
	move DESTINY_BOND, EFFECT_DESTINY_BOND,        0, GHOST,       	PHYSICAL, 100,  5,   0
	move HEX, 		   EFFECT_ALWAYS_HIT,         65, GHOST,	    SPECIAL, 100, 20,   0
	move LICK,         EFFECT_PARALYZE_HIT,       30, GHOST,        PHYSICAL, 100, 30,  30
	move MEAN_LOOK,    EFFECT_MEAN_LOOK,           0, GHOST,        STATUS, 100,  5,   0
	move NIGHT_SHADE,  EFFECT_LEVEL_DAMAGE,        1, GHOST,        SPECIAL, 100, 15,   0
	move PHANTOMFORCE, EFFECT_FLY,                100, GHOST,       PHYSICAL, 100, 10,   0
	move SHADOW_BALL,  EFFECT_SP_DEF_DOWN_HIT,    80, GHOST,        SPECIAL, 100, 15,  20
	move SHADOW_CLAW,  EFFECT_NORMAL_HIT,         75, GHOST,        PHYSICAL, 100, 20,   0
	move SHADOW_PUNCH, EFFECT_ALWAYS_HIT,         60, GHOST,	    PHYSICAL, 100, 20,   0
	move DRAGON_SWIPE, EFFECT_ATTACK_DOWN_HIT,    65, DRAGON,       PHYSICAL, 100, 20,  10
	move DRAGONBREATH, EFFECT_PARALYZE_HIT,       60, DRAGON,       SPECIAL, 100, 20,  30
	move DRAGON_CLAW,  EFFECT_NORMAL_HIT,         80, DRAGON,       PHYSICAL, 100, 15,  0
	move DRAGON_DANCE, EFFECT_DRAGON_DANCE,        0, DRAGON,	    STATUS, 100, 20,   0
	move DRAGON_PULSE, EFFECT_NORMAL_HIT,         90, DRAGON,       SPECIAL, 100, 15,  0
	move OUTRAGE,      EFFECT_RAMPAGE,           120, DRAGON,       PHYSICAL, 100, 15,   0
	move TWISTER,      EFFECT_TWISTER,            40, DRAGON,       SPECIAL, 100, 20,  20
	move BITE,         EFFECT_FLINCH_HIT,         60, DARK,         PHYSICAL, 100, 25,  30
	move CRUNCH,       EFFECT_DEFENSE_DOWN_HIT,   80, DARK,         PHYSICAL, 100, 15,  20
	move DARK_PULSE,   EFFECT_FLINCH_HIT,         80, DARK,         SPECIAL, 100, 15,   20
	move FAINT_ATTACK, EFFECT_ALWAYS_HIT,         60, DARK,         PHYSICAL, 100, 20,   0
	move THIEF,        EFFECT_THIEF,              60, DARK,         PHYSICAL, 100, 10, 100
	move NASTY_PLOT,   EFFECT_SP_ATK_UP_2,         0, DARK,         STATUS, 100, 30,   0
	move NIGHT_SLASH,  EFFECT_NORMAL_HIT,         70, DARK,         PHYSICAL, 100, 20,   0
	move SNARL,        EFFECT_SP_ATK_DOWN_HIT,    55, DARK,         SPECIAL, 100, 10,  100
	move BULLET_PUNCH, EFFECT_PRIORITY_HIT,       40, STEEL,        PHYSICAL, 100, 30,   0
	move CUT,          EFFECT_NORMAL_HIT,         65, STEEL,        PHYSICAL, 95, 30,   0
	move FLASH_CANNON, EFFECT_SP_DEF_DOWN_HIT,    80, STEEL,        SPECIAL, 100, 10,  10
	move IRON_HEAD,    EFFECT_NORMAL_HIT,         80, STEEL,        PHYSICAL, 100, 10,   0  
	move METAL_CLAW,   EFFECT_ATTACK_UP_HIT,      50, STEEL,        PHYSICAL, 95, 35,  10
	move METEOR_MASH,  EFFECT_ATTACK_UP_HIT,      90, STEEL,		PHYSICAL, 90, 10,	10
	move MIRROR_SHOT,  EFFECT_ACCURACY_DOWN_HIT,  65, STEEL,        SPECIAL, 85, 10,  20
	move STEEL_WING,   EFFECT_DEFENSE_UP_HIT,     80, STEEL,        PHYSICAL, 95, 25,  10
	move CHARM,        EFFECT_ATTACK_DOWN_2,       0, FAIRY,        STATUS, 100, 20,   0
	move DAZZLINGLEAM, EFFECT_NORMAL_HIT,         80, FAIRY,        SPECIAL, 100, 15,   0
	move DRAININGKISS, EFFECT_LEECH_HIT,          50, FAIRY,        SPECIAL, 100, 15,   0
	move FAIRY_WIND,   EFFECT_NORMAL_HIT,         40, FAIRY,        SPECIAL, 100, 25,   0
	move LOVELY_KISS,  EFFECT_SLEEP,               0, FAIRY,        STATUS, 75, 10,   0
	move MOONBLAST,	   EFFECT_SP_ATK_DOWN_HIT,    95, FAIRY,        SPECIAL, 100, 15,  10
	move PLAYROUGH,	   EFFECT_ATTACK_DOWN_HIT,    90, FAIRY,		PHYSICAL, 90, 10,	10
	move CURSE,        EFFECT_CURSE,               0, CURSE_TYPE,   STATUS, 100, 10,   0
	move SPLASH,       EFFECT_SPLASH,              0, NORMAL,       STATUS, 100, 20,   0
	move STRUGGLE,     EFFECT_RECOIL_HIT,         50, NORMAL,      	PHYSICAL, 100,  1,   0
	move BEAT_UP,      EFFECT_BEAT_UP,            20, DARK,         PHYSICAL, 100, 10,   0













































































	
